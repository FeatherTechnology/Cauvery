$(document).ready(function () {
  ...
  $("#staff_dept_name").change(function () {
    // To show Team Name.
  ...
  $(document).on("click", "#submitConDepName", function () {
    var con_dep_name = $("#con_dep_name").val().trim();
    ...
    if (con_dep_name != "") {
      $.ajax({
        ...
        type: "POST",
        data: {
          con_dep_name: con_dep_name,
          con_dep_name_id: con_dep_name_id,
        },
        cache: false,
        success: function (response) {
...
function getDeptName() {
  ...
  $.ajax({
    ...
    cache: false,
    success: function (response) {
      //showing all department no Restriction based on company.
      $("#staff_dept_name").empty();
      $("#staff_dept_name").append(
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < len; i++) {
        ...
        $("#ag_name").append(
          "<option value='" + ag_id + "'> " + name + " </option>",
        );
      }
    },
  });
...
function resetconcernDepartmentModal() {
  $("#con_dep_name").val("");
  $("#con_dep_name_id").val("");
  $("#conDepNameCheck").hide();
}

function getconTeamName() {
  // To show Team Name.
  var companyID = $("#company_id").val();
  $.ajax({
