...
$(document).ready(function () {
  ...
  $("#submit_staff_creation").click(function () {
    ...
    if (submitValidation()) {
      let confirmAction = confirm(
        "Are you sure you want to submit Staff Creation?",
      );
      if (!confirmAction) {
  ...
  // <--- Edit Departnent Name --->
  $(document).on("click", ".editDepName", function () {
    var id = $(this).data("id");
    var name = $(this).data("name");
    $("#department_name_id").val(id);
    $("#department_name").val(name);
  });

  // <--- Delete Departnent Name --->
      ...
      if (result.isConfirmed) {
        ...
        $.ajax({
          ...
          error: function (error) {
            Swal.fire({
              title: "Error!",
              text: "Something went wrong while deleting the Department Name.",
              icon: "error",
  ...
  // <--- Submit Departnent Name --->
  $(document).on("click", "#submitDepartmentName", function () {
    ...
    if (department_name != "") {
      $.ajax({
        ...
        type: "POST",
        data: {
          con_dep_name: department_name,
          con_dep_name_id: department_name_id,
        },
        cache: false,
        success: function (response) {
          var existResult = response.includes("Exists");
          ...
          if (existResult) {
            ...
            Swal.fire({
              title: "Error!",
              text: "Something went wrong while submitting the department name.",
              icon: "error",
              showConfirmButton: true,
              confirmButtonColor: "#0C70AB",
              confirmButtonText: "OK",
  ...
  // <--- Delete Team Name --->
  $(document).on("click", ".deleteTeamName", function () {
    var id = $(this).data("id");
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        // AJAX call
        $.ajax({
          url: "staffCreation/deleteTeamName.php",
          type: "POST",
          data: {
            id: id,
          },
          ...
          success: function (result) {
            if (result.response.includes("Successfully")) {
              Swal.fire({
                title: "Success!",
                text: "Team Name Deleted Successfully..!",
                icon: "success",
                showConfirmButton: true,
                confirmButtonColor: "#0C70AB",
                confirmButtonText: "OK",
          ...
          error: function (error) {
            Swal.fire({
              title: "Error!",
              text: "Something went wrong while deleting the Team Name.",
              icon: "error",
              confirmButtonColor: "#cc4444",
            });
          },
  ...
  // <--- Submit Team Name --->
  $(document).on("click", "#submitTeamName", function () {
    var team_name = $("#team_name").val().trim();
    var team_name_id = $("#team_name_id").val();

    if (team_name != "") {
      $.ajax({
        url: "staffCreation/submitTeamName.php",
        type: "POST",
        data: {
          team_name: team_name,
        ...
        success: function (response) {
          ...
          if (existResult) {
            ...
            Swal.fire({
              title: "Success!",
              text: "Team Name Added Successfully..!",
              icon: "success",
              showConfirmButton: true,
              confirmButtonColor: "#0C70AB",
              confirmButtonText: "OK",
            ...
            Swal.fire({
              title: "Error!",
              text: "Something went wrong while submitting the team name.",
              icon: "error",
              showConfirmButton: true,
              confirmButtonColor: "#0C70AB",
              confirmButtonText: "OK",
      ...
      buttons: [
        {
          ...
          action: function (e, dt, button, config) {
            ...
            var dynamic = curDateJs("Staff_type_List"); // or any base
...
function getDepartmentName() {
  $.ajax({
    ...
    success: function (response) {
      ...
      // Select previously saved department
      $("#department").val($("#department_upd").val());
    },
  });
}
...
function getDepartmentNameList() {
  $.ajax({
    ...
    success: function (response) {
      // Clear old rows
      $("#DepartmentNameTable tbody").empty();

      for (var i = 0; i < response.length; i++) {
        $("#DepartmentNameTable tbody").append(
          "<tr>" +
            "<td>" +
            (i + 1) +
            "</td>" +
...
function getTeamName() {
  $.ajax({
    url: "staffCreation/getTeamName.php",
    type: "POST",
    data: {},
    dataType: "json",
    success: function (response) {
      $("#team").empty();
      $("#team").append("<option value=''>Select Team Name</option>");
      ...
      // Select previously saved team
      $("#team").val($("#team_upd").val());
    },
  });
}
...
function getTeamNameList() {
  $.ajax({
    ...
    success: function (response) {
      // Clear old rows
      $("#TeamNameTable tbody").empty();

      for (var i = 0; i < response.length; i++) {
        $("#TeamNameTable tbody").append(
          "<tr>" +
            "<td>" +
            (i + 1) +
            "</td>" +
            ...
            "</td>" +
            "<td>" +
            "<span class='icon-border_color editTeamName' " +
            "data-id='" +
            response[i].id +
            "' " +
            "data-name='" +
            ...
            "style='cursor:pointer; margin-right:10px;' " +
            "title='Edit'></span>" +
            "<span class='icon-trash-2 deleteTeamName' " +
            "data-id='" +
            response[i].id +
            "' " +
            "style='cursor:pointer;' " +
...
// <--- Reset Modal --->
function resetModal(inputId, hiddenId, validationId) {
  $("#" + inputId).val("");
  $("#" + hiddenId).val("");
  $("#" + validationId).hide();
}
