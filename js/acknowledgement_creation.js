...
$(document).ready(function () {
  ...
  $(document).on("click", "#add_cus_label", function () {
    $("#feedback_label, #cus_feedback_department, #cus_feedback").prop(
      "disabled",
      false,
    );
    getFeedbackLable();
  ...
  $("#add_sign_doc").click(function () {
    $("#signDocUploads input")
      .not("#signType_cus_name, #guar_name")
      .prop("readonly", false);
  ...
  $("#signDocUploads").on("submit", function (e) {
    ...
    if (
      ...
      doc_Count != "" &&
      req_id != "" &&
      (sign_type == "2" || sign_type == "3"
        ? signType_relationship != ""
        : true)
    ) {
      $.ajax({
        type: "POST",
  ...
  $("#add_Cheque").click(function () {
    $("#chequeUploads input")
      .not("#holder_name, #cheque_relation")
      .prop("readonly", false);
  ...
  $("#chequeUploads").on("submit", function (e) {
    ...
    $(".chequeno").each(function () {
      ...
      checkChequeNo =
        $(this).val() == "" || checkChequeNo == false ? false : true;
      i++;
    });
    ...
    if (
      ...
      req_id != "" &&
      checkChequeNo == true &&
      (holder_type == "2" ? holder_relationship_name != "" : true)
    ) {
      $.ajax({
        type: "POST",
...
$(function () {
  ...
  $("input")
    .not("#int_rate, #due_period, #doc_charge, #proc_fee")
    .attr("readonly", true);
  $("select")
    .not(
      "#choose_document, #mortgage_process, #endorsement_process, #replace_doc_id",
    )
    .attr("disabled", true);
...
function getDocumentIds() {
  ...
  $.post(
    "verificationFile/documentation/getDoumentIDsTillHandover.php",
    { cusid },
    function (response) {
...
function getImage() {
  ...
  if (guarentorimg != "") {
    $("#imgshows").attr(
      "src",
      "uploads/verification/guarentor/" + guarentorimg + " ",
    );
  }
}
...
function getDistrictDropdown(StateSelected) {
  ...
  for (var i = 0; i < optionsList.length; i++) {
    var selected = "";
    if (
      district_upd != undefined &&
      district_upd != "" &&
      district_upd == optionsList[i]
    ) {
      selected = "selected";
    }
    htmlString =
...
function resetfeedback() {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      $(
        "#feedback_label, #cus_feedback_department, #cus_feedback, #feedback_remark, #feedbackID, #customer_summary_uploads",
      ).val("");
    },
  });
}
...
function getFeedbackLable() {
  $.post(
    ...
    function (data) {
      $("#feedback_label")
        .empty()
        .append("<option value=''>Select Feedback Label</option>");

      for (var i = 0; i < data.length; i++) {
        var feedback_name = data[i]["feedback_name"];
        var id = data[i]["id"];
        $("#feedback_label").append(
          "<option value='" + id + "'>" + feedback_name + "</option>",
        );
      }
    },
    "json",
...
//////////////////////////////////////////////////// Documentation  Start////////////////////////////////////////
function onLoadDocEditFunction() {
  //On load for Loan Calculation edit
    ...
    success: function (response) {
      if (cus_type == "Existing") {
        if (response.length != 0) {
          //check json response is not empty
  ...
  $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < len - 1; i++) {
        ...
        $("#ownername_relationship_name").append(
          `<option value='${fam_id}' ${selected}>${fam_name}</option>`,
        );
      }
...
function resetsignInfo() {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      $(
        "#sign_type, #signType_cus_name, #guar_name, #signType_relationship, #doc_Count, #signdoc_upd, #signedID",
      ).val("");
      $(
        "#cus_name_div, #guar_name_div, #docNameCheck, #signTypeCheck, #docCountCheck, #docupdCheck, #signTyperRelationshipCheck",
      ).hide();
    },
  });
}
...
function resetchequeInfo() {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      $(
        "#holder_type, #holder_name, #holder_relationship_name, #cheque_relation, #chequebank_name, #cheque_count, #cheque_upd, #chequeID",
      ).val("");
      $(
        "#chequebankCheck, #holdertypeCheck, #chequeCountCheck, #chequeupdCheck, #holderNameCheck",
      ).hide();
    },
  });
}
...
function resetgoldInfo() {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      $(
        "#gold_sts, #gold_type, #Purity, #gold_Count, #gold_Weight, #gold_Value, #gold_upload, #goldID",
      ).val("");
      $(
...
$("#add_document").click(function () {
  $("#docUploads input")
    .not("#docholder_name, #doc_relation")
    .prop("readonly", false);
...
$("#docInfoBtn").click(function () {
  ...
  if (
    ...
    doc_holder != "" &&
    relation != "" &&
    (doc_holder === "2" ? relation_name !== "" : true)
  ) {
    $.ajax({
...
function resetdocInfo() {
  ...
  $.ajax({
    ...
    success: function (html) {
      ...
      $(
        "#document_name, #document_details, #docholder_relationship_name, #document_type, #document_holder, #doc_info_id, #docholder_name, #relation_name, #doc_relation, #document_info_upd",
      ).val("");
      $(
        "#documentnameCheck, #documentdetailsCheck, #documentTypeCheck, #docholderCheck, #docHolderNameCheck",
      ).hide();
    },
  });
}
...
function doc_submit_validation(event) {
  ...
  var Propertyholder_relationship_name = $(
    "#Propertyholder_relationship_name",
  ).val();
  var mortgage_document_upd = $("#mortgage_document_upd").val();
  var mortgage_old_doc_upd = $("#mortgage_doc_upd").val();
  var RC_document_upd = $("#RC_document_upd").val();
  var RC_old_document_upd = $("#rc_doc_upd").val();

  var mortgage_name = $("#mortgage_name").val();
  var mortgage_dsgn = $("#mortgage_dsgn").val();
  var mortgage_nuumber = $("#mortgage_nuumber").val();
  var reg_office = $("#reg_office").val();
  var mortgage_value = $("#mortgage_value").val();
  var mortgage_document = $("#mortgage_document").val();

  var endorsement_name = $("#endorsement_name").val();
  var en_RC = $("#en_RC").val();
  ...
  if (mortgage_process == "0") {
    ...
    if (mortgage_document == "") {
      // check if document dropdown selected or not
      event.preventDefault();
      $("#mortgagedocCheck").show();
      validation = false;
    } else {
      //if selected then check if document dropdown is YES(0) then check for document uploaded now or already
...
$("#submit_loan_calculation").click(function (e) {
  ...
  performLoanCalculation(function () {
    ...
    if (isValid) {
      let confirmAction = confirm(
        "Are you sure you want to submit Loan Calculation?",
      );
      if (confirmAction) {
  ...
  return $.ajax({
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        var selected = "";
        if (loan_category_upd == "" || loan_category_upd == undefined) {
          //if update is not available, then only use on load value of loan category
          if (
            loan_category != undefined &&
            loan_category != "" &&
            loan_category == response[i]["loan_category_id"]
          ) {
            selected = "selected";
          ...
          if (
            loan_category_upd != undefined &&
            loan_category_upd != "" &&
            loan_category_upd == response[i]["loan_category_id"]
          ) {
            selected = "selected";
            $("#loan_category_ack").val(response[i]["loan_category_id"]);
          }
        ...
        $("#moduleTable tbody tr").append(
          `<td><button type="button" tabindex='` +
            ...
            `' id="add_category_info[]" name="add_category_info" 
                class="btn btn-primary add_category_info" disabled>Add</button> </td><td><span class='icon-trash-2 ' id='' tabindex='` +
            tb +
            `'></span></td>
                </tr></tbody></table>`,
        );
...
function getLoaninfo(loan_cat) {
  let cus_id = $("#cus_id_loan").val();
  $.ajax({
    url: "requestFile/getLoanInfo.php",
    data: { loan_category_upd: loan_cat, cus_id: cus_id },
    dataType: "json",
    type: "post",
    cache: false,
    success: function (response) {
      if (response["advance"] == "Yes") {
        ...
        $("#ad_amt")
          .unbind("blur")
          .blur(function () {
            //To calculate loan amount and advance percentage
            var amt = $("#tot_value").val();
            var advance = $("#ad_amt").val();
...
function profitCalculationInfo() {
  ...
  if (profit_type != "") {
    //Call only if profit type autamatically set
    profitCalAjax(profit_type, loan_cat); //Call for edit
  }
  if (due_method != "") {
    //Call only if due method autamatically set
  ...
  setTimeout(function () {
    ...
    if (scheme_name != "") {
      //Call only if scheme name autamatically set
  ...
  $("#due_method_scheme").change(function () {
    ...
    if (due_method == "2") {
      // show weekdays only if weekly due method selected
      $(".day_scheme").show();
    } else {
      $(".day_scheme").hide();
    }
...
function profitCalAjax(profit_type, loan_cat) {
  ...
  if ($("#due_period_upd").val()) {
    var due_period_upd = $("#due_period_upd").val();
  } else {
    var due_period_upd = "";
  }
  if ($("#doc_charge_upd").val()) {
    var doc_charge_upd = $("#doc_charge_upd").val();
  } else {
    var doc_charge_upd = "";
  }
  if ($("#proc_fee_upd").val()) {
    var proc_fee_upd = $("#proc_fee_upd").val();
  } else {
    var proc_fee_upd = "";
  }
  if (profit_type == "1") {
    ...
    $(".scheme-calculation").hide();
    $.ajax({
      // To show profit calculation infos based on sub category
        ...
        if (response["due_type"] == "emi") {
          ...
          for (var i = 0; i < profit_method.length; i++) {
            if (profit_method[i] == "pre_intrest") {
              valuee = "Pre Benefit";
            } else if (profit_method[i] == "after_intrest") {
          ...
          $("#int_rate").attr(
            "onChange",
            `let val = parseFloat($(this).val());let min = ${parseFloat(response["intrest_rate_min"])};let max = ${parseFloat(response["intrest_rate_max"])}; if (!isNaN(val)) { if (val > max) {alert("Enter Lesser Value"); $(this).val(""); } else if (val < min) { alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
          ...
          $("#due_period").attr(
            "onChange",
            `let val = parseFloat($(this).val());let min = ${parseFloat(response["due_period_min"])};let max = ${parseFloat(response["due_period_max"])}; if (!isNaN(val)) { if(val > max){ alert("Enter Lesser Value"); $(this).val(""); } else if(val < min){ alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
          ); //To check value between rage
          ...
          $("#doc_charge").attr(
            "onChange",
            `let val = parseFloat($(this).val());let min = ${parseFloat(response["document_charge_min"])};let max = ${parseFloat(response["document_charge_max"])}; if (!isNaN(val)) { if(val > max){ alert("Enter Lesser Value"); $(this).val(""); } else if(val < min){ alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
          );
          ...
          $("#proc_fee").attr(
            "onChange",
            `let val = parseFloat($(this).val());let min = ${parseFloat(response["processing_fee_min"])};let max = ${parseFloat(response["processing_fee_max"])}; if (!isNaN(val)) { if(val > max){ alert("Enter Lesser Value"); $(this).val(""); } else if(val < min){ alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
          );
...
function schemeAjax(due_method, loan_cat) {
  ...
  $.ajax({
    //To show scheme names based on sub category
    ...
    success: function (response) {
      ...
      for (var i = 0; i < response.length; i++) {
        ...
        if (
          scheme_upd != "" &&
          scheme_upd != undefined &&
          scheme_upd == response[i]["scheme_id"]
        ) {
          selected = "selected";
...
function schemeCalAjax(scheme_id) {
  if (scheme_id != "") {
    ...
    $.ajax({
      //show scheme based loan info using scheme id
      ...
      success: function (response) {
        ...
        for (var i = 0; i < profit_method.length; i++) {
          ...
          var selected = "";
          if (
            scheme_profit_method_upd != "" &&
            scheme_profit_method_upd != undefined &&
            scheme_profit_method_upd == profit_method[i]
          ) {
            selected = "selected";
          }
          $("#scheme_profit_method").append(
        ...
        $("#due_period").attr("readonly", true); // setting readonly due to fixed due period
        ...
        $("#int_rate").attr(
          "onChange",
          `let val = parseFloat($(this).val());let min = ${parseFloat(response["intreset_min"])};let max = ${parseFloat(response["intreset_max"])}; if (!isNaN(val)) { if (val > max) {alert("Enter Lesser Value"); $(this).val(""); } else if (val < min) { alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
        ); //To check value between rage
        ...
        $("#doc_charge").attr(
          "onChange",
          `let val = parseFloat($(this).val());let min = ${parseFloat(response["doc_charge_min"])};let max = ${parseFloat(response["doc_charge_max"])}; if (!isNaN(val)) { if(val > max){ alert("Enter Lesser Value"); $(this).val(""); } else if(val < min){ alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
        ); //To check value between rage
        ...
        $("#proc_fee").attr(
          "onChange",
          `let val = parseFloat($(this).val());let min = ${parseFloat(response["proc_fee_min"])};let max = ${parseFloat(response["proc_fee_max"])}; if (!isNaN(val)) { if(val > max){ alert("Enter Lesser Value"); $(this).val(""); } else if(val < min){ alert("Enter Higher Value"); $(this).val(""); } }else{ alert("Please enter a valid numeric value"); $(this).val("");}`,
        ); //To check value between rage
...
function getSchemeAfterIntreset() {
  ...
  if (intreset_type.includes("₹")) {
    ...
    var int_amt = (parseInt(loan_amt) * (parseFloat(int_rate) / 100)).toFixed(
      0,
