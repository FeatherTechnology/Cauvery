...
class admin
{
	...
	public function getuser($mysqli, $idupd)
	{
		...
		if ($mysqli->affected_rows > 0) {
			...
			$detailrecords['bank_access']              = strip_tags($row->bank_access);
			$detailrecords['ver_loan_cat']              = strip_tags($row->ver_loan_cat);
			$detailrecords['ver_group_id']              = strip_tags($row->ver_group_id);
			$detailrecords['app_loan_cat']              = strip_tags($row->app_loan_cat);
			$detailrecords['ack_loan_cat']              = strip_tags($row->ack_loan_cat);
			...
			$detailrecords['home_access']              = strip_tags($row->home_access);
			$detailrecords['cus_summary_access']              = strip_tags($row->cus_summary_access);
			$detailrecords['promotion_activity_mapping_access']   = strip_tags($row->promotion_activity_mapping_access);
			$detailrecords['promotion_activity_action_access']   = strip_tags($row->promotion_activity_action_access);
			$detailrecords['promotion_access']              = strip_tags($row->pro_aty_access);
			$detailrecords['repromotion_access']              = strip_tags($row->repro_aty_access);
			...
			$detailrecords['work_report_module']              = strip_tags($row->work_report_module);
			$detailrecords['monitor_report_module']              = strip_tags($row->monitor_report_module);
			$detailrecords['count_report_module']              = strip_tags($row->count_report_module);
			$detailrecords['analysis_report_module']              = strip_tags($row->analysis_report_module);
			$detailrecords['accounts_report_module']              = strip_tags($row->accounts_report_module);
			$detailrecords['request']              = strip_tags($row->request);
			$detailrecords['request_list_access']              = strip_tags($row->request_list_access);
			$detailrecords['req_mapping_access']              = strip_tags($row->req_mapping_access);
			...
			$detailrecords['acknowledgement_access']              = strip_tags($row->acknowledgement_access);
			...
			$detailrecords['noc_handover']              = strip_tags($row->noc_handover);
			$detailrecords['noc_replace']              = strip_tags($row->noc_replace);
			$detailrecords['doc_replace_remove_access']              = strip_tags($row->doc_replace_remove_access);
			$detailrecords['noc_replace_access']              = strip_tags($row->noc_replace_access);
			$detailrecords['noc_mapping_access']              = strip_tags($row->noc_mapping_access);
			...
			$detailrecords['update_cp_edit_access']              = strip_tags($row->update_cp_edit_access);
			$detailrecords['update_documentation']              = strip_tags($row->update_documentation);
			$detailrecords['update_doc_edit_access']              = strip_tags($row->update_doc_edit_access);
			...
			$detailrecords['bnk_clr_upl_acc']              = strip_tags($row->bnk_clr_upl_acc);
			$detailrecords['finance_insight']              = strip_tags($row->finance_insight);
			$detailrecords['hand_cash_balance_sheet']              = strip_tags($row->hand_cash_balance_sheet);
			$detailrecords['agent_balance_sheet']              = strip_tags($row->agent_balance_sheet);
			$detailrecords['accounts_loan_issue']              = strip_tags($row->accounts_loan_issue);
			...
			$detailrecords['repromotion_activity']              = strip_tags($row->repromotion_activity);
			...
			$detailrecords['due_nil_followup']              =  strip_tags($row->due_nil_followup);
			$detailrecords['ecs_followup']              = strip_tags($row->ecs_followup);
			...
			$detailrecords['verification_report'] = strip_tags($row->verification_report);
			$detailrecords['approval_report'] = strip_tags($row->approval_report);
			$detailrecords['cancel_revoke_report'] = strip_tags($row->cancel_revoke_report);
			...
			$detailrecords['in_closed_report'] = strip_tags($row->in_closed_report);
			$detailrecords['closed_report'] = strip_tags($row->closed_report);
			$detailrecords['confirmation_followup_report'] = strip_tags($row->confirmation_followup_report);
			$detailrecords['agent_report'] = strip_tags($row->agent_report);
			$detailrecords['no_due_pay_report'] = strip_tags($row->no_due_pay_report);
			$detailrecords['other_trans_report'] = strip_tags($row->other_trans_report);
			$detailrecords['day_end_report'] = strip_tags($row->day_end_report);
			$detailrecords['cash_tally_activity_report'] = strip_tags($row->cash_tally_activity_report);
			...
			$detailrecords['promotion_activity_report'] = strip_tags($row->promotion_activity_report);
			$detailrecords['cleared_report'] = strip_tags($row->cleared_report);
			$detailrecords['events_report'] = strip_tags($row->events_report);
			$detailrecords['area_loan_count_report'] = strip_tags($row->area_loan_count_report);
			...
			$detailrecords['concern_report'] = strip_tags($row->concern_report);
			$detailrecords['partners_report'] = strip_tags($row->partners_report);
			$detailrecords['request_count_report'] = strip_tags($row->request_count_report);
			$detailrecords['verification_count_report'] = strip_tags($row->verification_count_report);
			$detailrecords['approval_count_report'] = strip_tags($row->approval_count_report);
			$detailrecords['loan_issue_count_report'] = strip_tags($row->loan_issue_count_report);
			$detailrecords['promotion_count_report'] = strip_tags($row->promotion_count_report);
			$detailrecords['due_followup_count_report'] = strip_tags($row->due_followup_count_report);
			$detailrecords['back_office_count_report'] = strip_tags($row->back_office_count_report);
			$detailrecords['branch_request_count_report'] = strip_tags($row->branch_request_count_report);
			$detailrecords['location_track_report'] = strip_tags($row->location_track_report);
			$detailrecords['outstanding_report'] = strip_tags($row->outstanding_report);
	...
	public function addLoanCategoryDetails($mysqli, $userid)
	{
		...
		if (isset($_POST['monthly_due_method'])) {
			$due_method = $_POST['monthly_due_method'];
		}
		// if (isset($_POST['due_type'])) {
		// 	$due_type = $_POST['due_type'];
		// }
		// if (isset($_POST['monthly_profit_method'])) {
		// 	$profit_methods = $_POST['monthly_profit_method'];
		// 	$profit_method = implode(",", $profit_methods);
		// }
		if (isset($_POST['monthly_duetype'])) {
			$due_type = $_POST['monthly_duetype'];
		}
		if (isset($_POST['monthly_duetype']) && $_POST['monthly_duetype'] == 'emi') {
			// EMI → take from select (array)
			if (isset($_POST['monthly_profit_method'])) {
				$profit_methods = $_POST['monthly_profit_method'];  // array
				$profit_method = implode(",", $profit_methods); // store as CSV
			}
		} elseif (isset($_POST['monthly_duetype']) && $_POST['monthly_duetype'] == 'intrest') {
		...
		if (isset($_POST['monthly_intrests_rate_min'])) {
			$intrest_rate_min = $_POST['monthly_intrests_rate_min'];
		}
		if (isset($_POST['monthly_intrest_rate_max'])) {
			$intrest_rate_max = $_POST['monthly_intrest_rate_max'];
		}
		if (isset($_POST['monthly_due_period_min'])) {
			$due_period_min = $_POST['monthly_due_period_min'];
		}
		if (isset($_POST['monthly_due_period_max'])) {
			$due_period_max = $_POST['monthly_due_period_max'];
		}
		if (isset($_POST['monthly_doc_charges_type'])) {
			$doc_charge_type = $_POST['monthly_doc_charges_type'];
		}
		if (isset($_POST['monthly_document_charge_min'])) {
			$document_charge_min = $_POST['monthly_document_charge_min'];
		}
		if (isset($_POST['monthly_document_charge_max'])) {
			$document_charge_max = $_POST['monthly_document_charge_max'];
		}
		if (isset($_POST['proc_fees_type'])) {
			$proc_fee_type = $_POST['proc_fees_type'];
		}
		if (isset($_POST['monthly_processing_fee_min'])) {
			$processing_fee_min = $_POST['monthly_processing_fee_min'];
		}
		if (isset($_POST['monthly_processing_fee_max'])) {
			$processing_fee_max = $_POST['monthly_processing_fee_max'];
		}
		...
		if (isset($_POST['monthly_collection_info'])) {
			$collection_info = $_POST['monthly_collection_info'];
		}
		...
		if (!empty($_POST['monthly_intrests_rate_min'])) {

			$loanInsert = "INSERT INTO loan_calculation(loan_category_id, loan_category, due_method, due_type, profit_method, calculate_method, intrest_rate_min,
	    ...
	    overdue_type , overdue, collection_info, insert_login_id)
		VALUES('" . strip_tags($loan_category_id) . "','" . strip_tags($loan_category_name) . "', '" . strip_tags($due_method) . "','" . strip_tags($due_type) . "', '" . strip_tags($profit_method) . "', '" . strip_tags($calculate_method) . "',
		...
		'" . strip_tags($document_charge_max) . "','" . strip_tags($proc_fee_type) . "', '" . strip_tags($processing_fee_min) . "', '" . strip_tags($processing_fee_max) . "', '" . strip_tags($overdue_type) . "', '" . strip_tags($overdue) . "', '" . strip_tags($collection_info) . "', '" . strip_tags($userid) . "' )";
	...
	public function getLoanCategoryDetails($mysqli, $id)
	{
		...
		if ($mysqli->affected_rows > 0) {
			...
			$detailrecords['agent_loan']    = $row->agent_loan;
		}
	...
	public function updateLoanCategoryDetails($mysqli, $id, $userid)
	{
		...
		// if (isset($intrest_rate_max) && $_POST['loan_id_upd'] === '') {
		// 	$loanInsert = "INSERT INTO loan_calculation(`loan_category_id`,loan_category, due_method, due_type, profit_method, calculate_method, intrest_rate_min,
		// intrest_rate_max, due_period_min, due_period_max,doc_charge_type, document_charge_min, document_charge_max, proc_fee_type,processing_fee_min, processing_fee_max,
		// overdue_type ,overdue, collection_info, insert_login_id)
		// VALUES('" . strip_tags($id) . "','" . strip_tags($loan_category_name) . "', '" . strip_tags($due_method) . "','" . strip_tags($due_type) . "', '" . strip_tags($profit_method) . "', calculate_method = '" . strip_tags($calculate_method) . "',
		//  '" . strip_tags($intrest_rate_min) . "', '" . strip_tags($intrest_rate_max) . "',  '" . strip_tags($due_period_min) . "','" . strip_tags($due_period_max) . "', '" . strip_tags($doc_charge_type) . "','" . strip_tags($document_charge_min) . "',
		// '" . strip_tags($document_charge_max) . "','" . strip_tags($proc_fee_type) . "', '" . strip_tags($processing_fee_min) . "', '" . strip_tags($processing_fee_max) . "', overdue_type = '" . strip_tags($overdue_type) . "', '" . strip_tags($overdue) . "', '" . strip_tags($collection_info) . "', '" . strip_tags($userid) . "' )";
        ...
        intrest_rate_max, due_period_min, due_period_max, doc_charge_type, document_charge_min, document_charge_max, proc_fee_type, processing_fee_min, processing_fee_max,
        overdue_type, overdue, collection_info, insert_login_id)
        VALUES('" . strip_tags($cat_id) . "','" . strip_tags($loan_category_name) . "', '" . strip_tags($due_method) . "','" . strip_tags($due_type) . "', '" . strip_tags($profit_method) . "', '" . strip_tags($calculate_method) . "',
         '" . strip_tags($intrest_rate_min) . "', '" . strip_tags($intrest_rate_max) . "',  '" . strip_tags($due_period_min) . "','" . strip_tags($due_period_max) . "', '" . strip_tags($doc_charge_type) . "','" . strip_tags($document_charge_min) . "',
        '" . strip_tags($document_charge_max) . "','" . strip_tags($proc_fee_type) . "', '" . strip_tags($processing_fee_min) . "', '" . strip_tags($processing_fee_max) . "', '" . strip_tags($overdue_type) . "', '" . strip_tags($overdue) . "', '" . strip_tags($collection_info) . "', '" . strip_tags($userid) . "' )";
		...
		if (isset($_POST['scheme_name'])) {
			$selected_scheme_ids = array_map('intval', $_POST['scheme_name']);
			$scheme_id_str = implode(",", $selected_scheme_ids);
			$update_selected = "UPDATE `loan_scheme`SET `loan_category_id`= '$cat_id',`loan_category` = '$loan_category_name',`updated_date` = CURRENT_TIMESTAMP() WHERE `scheme_id` IN ($scheme_id_str);";
			$mysqli->query($update_selected) or die("Error (selected update): " . $mysqli->error);
		...
		for ($i = 0; $i <= sizeof($loan_category_ref_name) - 1; $i++) {
			...
			$rrUpdaet = "INSERT INTO loan_category_ref(loan_category_id, loan_category_ref_name) VALUES('" . strip_tags($cat_id) . "', '" . strip_tags($loan_category_ref_name[$i]) . "')";
	...
	public function addLoanCalculation($mysqli, $userid)
	{
		...
		intrest_rate_max, due_period_min, due_period_max,doc_charge_type, document_charge_min, document_charge_max, proc_fee_type, processing_fee_min, processing_fee_max, overdue_type , overdue, due_date, grace_period, penalty, collection_info, insert_login_id)
		VALUES('" . strip_tags($loan_category) . "', '" . strip_tags($sub_category) . "', '" . strip_tags($due_method) . "', 
		'" . strip_tags($due_type) . "', '" . strip_tags($profit_method) . "', '" . strip_tags($calculate_method) . "', 
		'" . strip_tags($intrest_rate_min) . "', '" . strip_tags($intrest_rate_max) . "',  '" . strip_tags($due_period_min) . "',
		'" . strip_tags($due_period_max) . "', '" . strip_tags($doc_charge_type) . "','" . strip_tags($document_charge_min) . "',
		'" . strip_tags($document_charge_max) . "','" . strip_tags($proc_fee_type) . "', '" . strip_tags($processing_fee_min) . "',
		'" . strip_tags($processing_fee_max) . "', '" . strip_tags($overdue_type) . "', '" . strip_tags($overdue) . "',
		'" . strip_tags($due_date) . "','" . strip_tags($grace_period) . "','" . strip_tags($penalty) . "',  '" . strip_tags($collection_info) . "', 
		'" . strip_tags($userid) . "' )";
		$insresult = $mysqli->query($loanInsert) or die("Error " . $mysqli->error);
	}
	...
	public function getLoanCalculation($mysqli, $id)
	{
		...
		if ($mysqli->affected_rows > 0) {
			...
			$detailrecords['overdue_type']            = $row->overdue_type;
	...
	intrest_rate_min = '" . strip_tags($intrest_rate_min) . "', intrest_rate_max = '" . strip_tags($intrest_rate_max) . "',  
	due_period_min = '" . strip_tags($due_period_min) . "', due_period_max = '" . strip_tags($due_period_max) . "', doc_charge_type = '" . strip_tags($doc_charge_type) . "', document_charge_min = '" . strip_tags($document_charge_min) . "', document_charge_max = '" . strip_tags($document_charge_max) . "',
	proc_fee_type = '" . strip_tags($proc_fee_type) . "', processing_fee_min = '" . strip_tags($processing_fee_min) . "', 
	processing_fee_max = '" . strip_tags($processing_fee_max) . "', overdue_type = '" . strip_tags($overdue_type) . "',  overdue = '" . strip_tags($overdue) . "',
	due_date = '" . strip_tags($due_date) . "', grace_period = '" . strip_tags($grace_period) . "', penalty = '" . strip_tags($penalty) . "', 
	collection_info = '" . strip_tags($collection_info) . "',
	...
	public function addAreaCreation($mysqli, $userid)
	{
		...
		$insertQry = "INSERT INTO area_creation(area_name_id,  taluk, district, state, pincode, insert_login_id,created_date	)
		VALUES('" . strip_tags($area_name_id) . "', '" . strip_tags($taluk) . "', '" . strip_tags($district) . "',
	...
	public function updateAreaCreation($mysqli, $id, $userid)
	{
		...
		$updateQry = "UPDATE area_creation set area_name_id='" . strip_tags($area_name_id) . "', taluk='" . strip_tags($taluk) . "', 
	...
	{
		$loanCatSelect = "SELECT loan_category_id, loan_category_name, status , agent_loan FROM loan_category GROUP BY loan_category_name";
		$res = $mysqli->query($loanCatSelect) or die("Error in Get All Records" . $mysqli->error);
		$detailrecords = array();
		if ($mysqli->affected_rows > 0) {
			$i = 0;
	...
	public function getLoanScheme($mysqli, $id)
	{
		...
		if ($res->num_rows > 0) {
			while ($row = $res->fetch_assoc()) {
				$detailrecords[] = $row; // Append each full row as an array
			}
		}
		return $detailrecords;
	}
	...
	public function addUser($mysqli, $userid)
	{
		...
		if (isset($_POST['bank_access_id'])) {
			$bank_access = $_POST['bank_access_id'];
		}
		$line = '';
		if (isset($_POST['line'])) {
			$line = $_POST['line'];
		...
		$repro_aty_access = '';
		if (isset($_POST['repro_aty_access_id'])) {
			$repro_aty_access = $_POST['repro_aty_access_id'];
		}
		$promotion_activity_mapping_access = '';
		if (isset($_POST['promotion_activity_mapping_access'])) {
			$promotion_activity_mapping_access = $_POST['promotion_activity_mapping_access'];
		}
		$promotion_activity_action_access = ($_POST['promotion_activity_action_access'] !='') ? $_POST['promotion_activity_action_access'] : '2';
		$download_access = '1';
		if (isset($_POST['download_access'])) {
			$download_access = $_POST['download_access'];
		}
		$report_access = '1';
		if (isset($_POST['report_access'])) {
			$report_access = $_POST['report_access'];
		}
		$home_access = '1';
		if (isset($_POST['home_access'])) {
			$home_access = $_POST['home_access'];
		}
		$cus_summary_access = ($_POST['cus_summary_access'] !='') ? $_POST['cus_summary_access'] : '1';
		...
		$req_mapping_access = '';
		if (isset($_POST['req_mapping_access'])) {
			$req_mapping_access = $_POST['req_mapping_access'];
		}
		...
		if (isset($_POST['verification']) &&    $_POST['verification'] == 'Yes') {
			$verification = 0;
			if (isset($_POST['ver_loan_cat'])) {
				$ver_loan_cat = $_POST['ver_loan_cat'];
			}
			if (isset($_POST['ver_group'])) {
				$ver_group_id = $_POST['ver_group'];
			}
		} else {
			$verification = 1;
			$ver_loan_cat = '';
		...
		if (isset($_POST['approval']) &&    $_POST['approval'] == 'Yes') {
			$approval = 0;
			if (isset($_POST['app_loan_cat'])) {
				$app_loan_cat = $_POST['app_loan_cat'];
			}
		} else {
			$approval = 1;
			$app_loan_cat = '';
		...
		if (isset($_POST['acknowledgement_access']) &&    $_POST['acknowledgement_access'] == 'Yes') {
			$acknowledgement_access = 0;
		} else {
		...
		if (isset($_POST['noc_handover']) &&    $_POST['noc_handover'] == 'Yes') {
			$noc_handover = 0;
		} else {
			$noc_handover = 1;
		}
		if (isset($_POST['noc_replace_access']) && $_POST['noc_replace_access'] == 'Yes') {
			$noc_replace_access = 0;
		} else {
			$noc_replace_access = 1;
		}
		$noc_mapping_access = '';
		if (isset($_POST['noc_mapping_access'])) {
			$noc_mapping_access = $_POST['noc_mapping_access'];
		}
		...
		if (isset($_POST['noc_replace']) && $_POST['noc_replace'] == 'Yes') {
			$noc_replace = 0;
		} else {
			$noc_replace = 1;
		}
		
		$doc_replace_remove_access = (isset($_POST['doc_replace_remove_access']) && $_POST['doc_replace_remove_access'] !='') ? $_POST['doc_replace_remove_access'] : '2';
		...
		if (isset($_POST['update']) && $_POST['update'] == 'Yes') { //Customer Profile.
			$update_screen = 0;
		} else {
			$update_screen = 1;
		}

		$update_cp_edit_access = ($_POST['update_cp_edit_access_id']) ? $_POST['update_cp_edit_access_id'] : '0';

		if (isset($_POST['update_documentation']) && $_POST['update_documentation'] == 'Yes') { //Documentation.
			$update_documentation = 0;
		} else {
		...
		$bnk_clr_upl_acc ='1';
		if (isset($_POST['bnk_clr_upl_acc']) && isset($_POST['accountsmodule']) &&    $_POST['accountsmodule'] == 'Yes') {
			$bnk_clr_upl_acc = $_POST['bnk_clr_upl_acc'];
		}
		...
		if (isset($_POST['hand_cash_balance_sheet']) &&    $_POST['hand_cash_balance_sheet'] == 'Yes') {
			$hand_cash_balance_sheet = 0;
		} else {
			$hand_cash_balance_sheet = 1;
		}
		if (isset($_POST['agent_balance_sheet']) &&    $_POST['agent_balance_sheet'] == 'Yes') {
			$agent_balance_sheet = 0;
		} else {
			$agent_balance_sheet = 1;
		}
		if (isset($_POST['accounts_loan_issue']) &&    $_POST['accounts_loan_issue'] == 'Yes') {
			$accounts_loan_issue = 0;
		} else {
			$accounts_loan_issue = 1;
		}
		...
		if (isset($_POST['repromotion_activity']) &&    $_POST['repromotion_activity'] == 'Yes') {
			$repromotion_activity = 0;
		} else {
			$repromotion_activity = 1;
		}
		...
		if (isset($_POST['due_nil_followup']) &&    $_POST['due_nil_followup'] == 'Yes') {
			$due_nil_followup = 0;
		} else {
			$due_nil_followup = 1;
		}
		...
		if (isset($_POST['work_report_module']) &&    $_POST['work_report_module'] == 'Yes' && isset($_POST['reportmodule']) &&    $_POST['reportmodule'] == 'Yes') {
			$work_report_module = 0;
		} else {
			$work_report_module = 1;
		}
		if (isset($_POST['monitor_report_module']) &&    $_POST['monitor_report_module'] == 'Yes' && isset($_POST['reportmodule']) &&    $_POST['reportmodule'] == 'Yes') {
			$monitor_report_module = 0;
		} else {
			$monitor_report_module = 1;
		}
		if (isset($_POST['count_report_module']) && $_POST['count_report_module'] == 'Yes' && isset($_POST['reportmodule']) && $_POST['reportmodule'] == 'Yes') {
			$count_report_module = 0;
		} else {
			$count_report_module = 1;
		}
		if (isset($_POST['analysis_report_module']) &&    $_POST['analysis_report_module'] == 'Yes' && isset($_POST['reportmodule']) &&    $_POST['reportmodule'] == 'Yes') {
			$analysis_report_module = 0;
		} else {
			$analysis_report_module = 1;
		}
		if (isset($_POST['accounts_report_module']) &&    $_POST['accounts_report_module'] == 'Yes' && isset($_POST['reportmodule']) &&    $_POST['reportmodule'] == 'Yes') {
			$accounts_report_module = 0;
		} else {
			$accounts_report_module = 1;
		}
		...
		if (isset($_POST['verification_report']) &&    $_POST['verification_report'] == 'Yes') {
			$verification_report = 0;
		} else {
			$verification_report = 1;
		}
		if (isset($_POST['approval_report']) &&    $_POST['approval_report'] == 'Yes') {
			$approval_report = 0;
		} else {
			$approval_report = 1;
		}
		if (isset($_POST['cancel_revoke_report']) &&    $_POST['cancel_revoke_report'] == 'Yes') {
			$cancel_revoke_report = 0;
		} else {
			$cancel_revoke_report = 1;
		}
		...
		if (isset($_POST['in_closed_report']) &&    $_POST['in_closed_report'] == 'Yes') {
			$in_closed_report = 0;
		} else {
			$in_closed_report = 1;
		}
		...
		if (isset($_POST['confirmation_followup_report']) &&    $_POST['confirmation_followup_report'] == 'Yes') {
			$confirmation_followup_report = 0;
		} else {
			$confirmation_followup_report = 1;
		}
		...
		if (isset($_POST['no_due_pay_report']) &&    $_POST['no_due_pay_report'] == 'Yes') {
			$no_due_pay_report = 0;
		} else {
			$no_due_pay_report = 1;
		}
		if (isset($_POST['other_trans_report']) &&    $_POST['other_trans_report'] == 'Yes') {
			$other_trans_report = 0;
		} else {
			$other_trans_report = 1;
		}
		...
		if (isset($_POST['cash_tally_activity_report']) &&    $_POST['cash_tally_activity_report'] == 'Yes') {
			$cash_tally_activity_report = 0;
		} else {
			$cash_tally_activity_report = 1;
		}
		if (isset($_POST['due_followup_customer_count_report']) &&    $_POST['due_followup_customer_count_report'] == 'Yes') {
			$due_followup_customer_count_report = 0;
		} else {
			$due_followup_customer_count_report = 1;
		}
		...
		if (isset($_POST['customer_status_report']) &&    $_POST['customer_status_report'] == 'Yes') {
			$customer_status_report = 0;
		} else {
			$customer_status_report = 1;
		}
		if (isset($_POST['promotion_activity_report']) &&    $_POST['promotion_activity_report'] == 'Yes') {
			$promotion_activity_report = 0;
		} else {
		...
		if (isset($_POST['cleared_report']) &&    $_POST['cleared_report'] == 'Yes') {
			$cleared_report = 0;
		} else {
			$cleared_report = 1;
		}
		...
		// if (isset($_POST['work_count_report']) &&    $_POST['work_count_report'] == 'Yes') {
		// 	$work_count_report = 0;
		// } else {
		// 	$work_count_report = 1;
		// }
		if (isset($_POST['noc_handover_report']) &&    $_POST['noc_handover_report'] == 'Yes') {
			$noc_handover_report = 0;
		} else {
			$noc_handover_report = 1;
		}
		if (isset($_POST['confirmation_count_report']) &&    $_POST['confirmation_count_report'] == 'Yes') {
			$confirmation_count_report = 0;
		} else {
			$confirmation_count_report = 1;
		}
		if (isset($_POST['concern_report']) &&    $_POST['concern_report'] == 'Yes') {
			$concern_report = 0;
		} else {
			$concern_report = 1;
		}
		if (isset($_POST['partners_report']) &&    $_POST['partners_report'] == 'Yes') {
			$partners_report = 0;
		} else {
			$partners_report = 1;
		}
		
		$request_count_report = (isset($_POST['request_count_report']) && $_POST['request_count_report'] == 'Yes') ? 0 : 1;
		$verification_count_report = (isset($_POST['verification_count_report']) && $_POST['verification_count_report'] == 'Yes') ? 0 : 1;
		$approval_count_report = (isset($_POST['approval_count_report']) && $_POST['approval_count_report'] == 'Yes') ? 0 : 1;
		$loan_issue_count_report = (isset($_POST['loan_issue_count_report']) && $_POST['loan_issue_count_report'] == 'Yes') ? 0 : 1;
		$promotion_count_report = (isset($_POST['promotion_count_report']) && $_POST['promotion_count_report'] == 'Yes') ? 0 : 1;
		$due_followup_count_report = (isset($_POST['due_followup_count_report']) && $_POST['due_followup_count_report'] == 'Yes') ? 0 : 1;
		$back_office_count_report = (isset($_POST['back_office_count_report']) && $_POST['back_office_count_report'] == 'Yes') ? 0 : 1;
		$branch_request_count_report = (isset($_POST['branch_request_count_report']) && $_POST['branch_request_count_report'] == 'Yes') ? 0 : 1;
		$location_track_report = (isset($_POST['location_track_report']) && $_POST['location_track_report'] == 'Yes') ? 0 : 1;
		$outstanding_report = (isset($_POST['outstanding_report']) && $_POST['outstanding_report'] == 'Yes') ? 0 : 1;

		if (isset($_POST['reportmodule_intrest']) &&    $_POST['reportmodule_intrest'] == 'Yes') {
			$reportmodule_intrest = 0;
		} else {
			$reportmodule_intrest = 1;
		}
		if (isset($_POST['intrest_ledger_report']) &&    $_POST['intrest_ledger_report'] == 'Yes') {
			$intrest_ledger_report = 0;
		} else {
			$intrest_ledger_report = 1;
		}
		if (isset($_POST['intrest_loan_issue_report']) &&    $_POST['intrest_loan_issue_report'] == 'Yes') {
			$intrest_loan_issue_report = 0;
		} else {
			$intrest_loan_issue_report = 1;
		}
		if (isset($_POST['intrest_collection_report']) &&    $_POST['intrest_collection_report'] == 'Yes') {
			$intrest_collection_report = 0;
		} else {
			$intrest_collection_report = 1;
		}
		if (isset($_POST['intrest_balance_report']) &&    $_POST['intrest_balance_report'] == 'Yes') {
			$intrest_balance_report = 0;
		} else {
			$intrest_balance_report = 1;
		}
		if (isset($_POST['intrest_closed_report']) &&    $_POST['intrest_closed_report'] == 'Yes') {
			$intrest_closed_report = 0;
		} else {
			$intrest_closed_report = 1;
		}
		...
		$insertQry = "INSERT INTO user(`fullname`, `emailid`, `user_name`, `user_password`, `role`, `role_type`, `dir_id`, `ag_id`, `staff_id`, `company_id`, `branch_id`, `bank_access`, `agentforstaff`, `line_id`, `group_id`, `download_access`, `report_access`, `home_access`, `cus_summary_access`, `pro_aty_access`, `repro_aty_access`,`promotion_activity_mapping_access`, `promotion_activity_action_access`, `mastermodule`, `company_creation`, `branch_creation`, `loan_category`, `area_creation`, `area_mapping`, `area_approval`, `adminmodule`, `director_creation`, `agent_creation`, `staff_creation`, `manage_user`, `bank_creation`, `requestmodule`, `request`, `request_list_access`,`req_mapping_access`,`verificationmodule`, `verification`, `ver_loan_cat`, `ver_group_id`, `approvalmodule`, `approval`, `app_loan_cat`, `acknowledgementmodule`, `acknowledgement`, `acknowledgement_access`, `ack_loan_cat`, `loanissuemodule`, `loan_issue`, `collectionmodule`, `collection`, `collection_access`, `closedmodule`, `closed`, `nocmodule`, `noc`, `noc_handover`, `noc_replace_access`, `noc_mapping_access`, `doctrackmodule`, `doctrack`, `doc_rec_access`, `noc_replace`, `doc_replace_remove_access`, `updatemodule`, `update_screen`, `update_cp_edit_access`, `update_documentation`, `update_doc_edit_access`, `concernmodule`, `concern_creation`, `concern_solution`, `accountsmodule`, `cash_tally`, `cash_tally_admin`, `bank_details`, `bank_clearance`, `bnk_clr_upl_acc`, `finance_insight`, `hand_cash_balance_sheet`,`agent_balance_sheet`,`accounts_loan_issue`, `followupmodule`, `promotion_activity`, `repromotion_activity`, `loan_followup`, `confirmation_followup`, `due_followup`, `due_nil_followup`,`ecs_followup`, `due_followup_lines`, `reportmodule`, `work_report_module`, `monitor_report_module`, `count_report_module`, `analysis_report_module`, `accounts_report_module`, `request_report`, `verification_report`, `approval_report`, `cancel_revoke_report`, `loan_issue_report`, `collection_report`, `principal_interest_report`, `balance_report`, `due_list_report`, `in_closed_report`, `closed_report`, `confirmation_followup_report`, `agent_report`, `no_due_pay_report`, `other_trans_report`, `day_end_report`, `cash_tally_activity_report`, `due_followup_customer_count_report`, `commitment_report`, `customer_status_report`, `promotion_activity_report`, `cleared_report`, `events_report`, `area_loan_count_report`, `noc_handover_report`, `confirmation_count_report`, `concern_report`, `partners_report`, `request_count_report`, `verification_count_report`, `approval_count_report`, `loan_issue_count_report`, `promotion_count_report`, `due_followup_count_report`,`back_office_count_report`, `branch_request_count_report`, `location_track_report`,`outstanding_report`, `reportmodule_intrest`, `intrest_ledger_report`, `intrest_loan_issue_report`, `intrest_collection_report`, `intrest_balance_report`, `intrest_closed_report`, `search_module`, `search`, `bulk_upload_module`, `bulk_upload`, `loan_track`, `sms_module`, `sms_generation`, `insert_login_id`, `created_date`) VALUES('$full_name', '$email', '$user_name', '$user_password', '$role', '$role_type', '$dir_name', '$ag_name', '$staff_name', '$company_id', '$branch_id', '$bank_access', '$agentforstaff', '$line', '$group', '$download_access', '$report_access', '$home_access', '$cus_summary_access', '$pro_aty_access','$repro_aty_access', '$promotion_activity_mapping_access', '$promotion_activity_action_access', '$mastermodule', '$company_creation', '$branch_creation', '$loan_category', '$area_creation', '$area_mapping', '$area_approval', '$adminmodule', '$director_creation', '$agent_creation', '$staff_creation', '$manage_user', '$bank_creation', '$requestmodule', '$request', '$request_list_access', '$req_mapping_access','$verificationmodule', '$verification', '$ver_loan_cat', '$ver_group_id', '$approvalmodule', '$approval', '$app_loan_cat', '$acknowledgementmodule', '$acknowledgement', '$acknowledgement_access', '$ack_loan_cat', '$loanissuemodule', '$loan_issue', '$collectionmodule', '$collection', '$collection_access', '$closedmodule', '$closed', '$nocmodule', '$noc', '$noc_handover', '$noc_replace_access', '$noc_mapping_access', '$doctrackmodule', '$doctrack', '$doc_rec_access', '$noc_replace', '$doc_replace_remove_access', '$updatemodule', '$update_screen', '$update_cp_edit_access', '$update_documentation', '$update_doc_edit_access', '$concernmodule', '$concernCreation', '$concernSolution', '$accountsmodule', '$cash_tally', '$cash_tally_admin', '$bank_details', '$bank_clearance', '$bnk_clr_upl_acc', '$finance_insight', '$hand_cash_balance_sheet','$agent_balance_sheet','$accounts_loan_issue', '$followupmodule', '$promotion_activity','$repromotion_activity','$loan_followup', '$conf_followup', '$due_followup','$due_nil_followup', '$ecs_followup', '$due_follup_line', '$reportmodule', '$work_report_module', '$monitor_report_module', '$count_report_module', '$analysis_report_module', '$accounts_report_module', '$request_report', '$verification_report', '$approval_report', '$cancel_revoke_report', '$loan_issue_report', '$collection_report', '$principal_interest_report', '$balance_report', '$due_list_report', '$in_closed_report', '$closed_report', '$confirmation_followup_report', '$agent_report', '$no_due_pay_report', '$other_trans_report', '$day_end_report', '$cash_tally_activity_report', '$due_followup_customer_count_report', '$commitment_report', '$customer_status_report', '$promotion_activity_report', '$cleared_report', '$events_report', '$area_loan_count_report', '$noc_handover_report', '$confirmation_count_report', '$concern_report', '$partners_report', '$request_count_report', '$verification_count_report', '$approval_count_report', '$loan_issue_count_report', '$promotion_count_report', '$due_followup_count_report', '$back_office_count_report', '$branch_request_count_report', '$location_track_report', '$outstanding_report', '$reportmodule_intrest', '$intrest_ledger_report', '$intrest_loan_issue_report', '$intrest_collection_report', '$intrest_balance_report', '$intrest_closed_report', '$searchmodule', '$search_screen', '$bulk_upload_module', '$bulk_upload', '$loan_track', '$sms_module', '$sms_generation', '$userid', now() )";
	...
	function updateUser($mysqli, $id, $user_id)
	{
		...
		$update_cp_edit_access = isset($_POST['update_cp_edit_access_id']) ? $_POST['update_cp_edit_access_id'] : '0';
		...
		$updateQry = "UPDATE `user` SET `fullname` = '$full_name', `emailid` = '$email', `user_name` = '$user_name', `user_password` = '$user_password', `role` = '$role', `role_type` = '$role_type', `dir_id` = '$dir_name', `ag_id` = '$ag_name', `staff_id` = '$staff_name', `company_id` = '$company_id', `branch_id` = '$branch_id', `bank_access` = '$bank_access', `agentforstaff` ='$agentforstaff', `line_id` = '$line', `group_id` = '$group', `download_access` = '$download_access', `report_access` = '$report_access', `home_access` = '$home_access', `cus_summary_access` = '$cus_summary_access', `pro_aty_access` = '$pro_aty_access', `repro_aty_access` = '$repro_aty_access',`promotion_activity_mapping_access` = '$promotion_activity_mapping_access', `promotion_activity_action_access` = '$promotion_activity_action_access', `mastermodule` = '$mastermodule', `company_creation` = '$company_creation', `branch_creation` = '$branch_creation', `loan_category` = '$loan_category', `area_creation` = '$area_creation', `area_mapping` = '$area_mapping', `area_approval` = '$area_approval', `adminmodule` = '$adminmodule', `director_creation` = '$director_creation', `agent_creation` = '$agent_creation', `staff_creation` = '$staff_creation', `manage_user` = '$manage_user', `bank_creation`='$bank_creation', `requestmodule`='$requestmodule', `request`='$request', `request_list_access`='$request_list_access',  `req_mapping_access`='$req_mapping_access',`verificationmodule`='$verificationmodule', `verification`='$verification', `ver_loan_cat`='$ver_loan_cat', `ver_group_id`='$ver_group_id', `approvalmodule`='$approvalmodule', `approval`='$approval', `app_loan_cat`='$app_loan_cat', `acknowledgementmodule`='$acknowledgementmodule', `acknowledgement`='$acknowledgement', `acknowledgement_access` = '$acknowledgement_access', `ack_loan_cat`='$ack_loan_cat', `loanissuemodule`='$loanissuemodule', `loan_issue`='$loan_issue', `collectionmodule` = '$collectionmodule', `collection` = '$collection', `collection_access` = '$collection_access', `closedmodule` = '$closedmodule', `closed` = '$closed', `nocmodule` = '$nocmodule', `noc` = '$noc', `noc_handover` = '$noc_handover', `noc_replace_access` = '$noc_replace_access', `noc_mapping_access`='$noc_mapping_access', `doctrackmodule` = '$doctrackmodule', `doctrack` = '$doctrack', `doc_rec_access` = '$doc_rec_access', `noc_replace` = '$noc_replace', `doc_replace_remove_access`= '$doc_replace_remove_access', `updatemodule` = '$updatemodule', `update_screen` = '$update_screen', `update_cp_edit_access` = '$update_cp_edit_access', `update_documentation` = '$update_documentation', `update_doc_edit_access` = '$update_doc_edit_access', `concernmodule`='$concernmodule', `concern_creation`='$concernCreation', `concern_solution`='$concernSolution', `accountsmodule`='$accountsmodule', `cash_tally`='$cash_tally', `cash_tally_admin`='$cash_tally_admin', `bank_details`='$bank_details', `bank_clearance`='$bank_clearance', `bnk_clr_upl_acc`='$bnk_clr_upl_acc', `finance_insight`='$finance_insight', `hand_cash_balance_sheet` ='". strip_tags($hand_cash_balance_sheet) ."' ,`agent_balance_sheet` ='". strip_tags($agent_balance_sheet) ."' , `accounts_loan_issue`='$accounts_loan_issue', `followupmodule`='$followupmodule', `promotion_activity`='$promotion_activity', `repromotion_activity`='$repromotion_activity', `loan_followup`='$loan_followup', `confirmation_followup`='$conf_followup', `due_followup`='$due_followup',`due_nil_followup`='$due_nil_followup', `ecs_followup`='$ecs_followup', `due_followup_lines`='$due_follup_line', `reportmodule` = '$reportmodule', `work_report_module` = '$work_report_module', `monitor_report_module` = '$monitor_report_module', `count_report_module` = '$count_report_module', `analysis_report_module` = '$analysis_report_module', `accounts_report_module` = '$accounts_report_module', `request_report` = '$request_report', `verification_report` = '$verification_report', `approval_report` = '$approval_report', `cancel_revoke_report` = '$cancel_revoke_report', `loan_issue_report` = '$loan_issue_report', `collection_report` = '$collection_report', `principal_interest_report` = '$principal_interest_report', `balance_report` = '$balance_report', `due_list_report` = '$due_list_report', `in_closed_report` = '$in_closed_report', `closed_report` = '$closed_report', `confirmation_followup_report` = '$confirmation_followup_report', `agent_report` = '$agent_report', `no_due_pay_report` = '$no_due_pay_report', `other_trans_report` = '$other_trans_report', `day_end_report` = '$day_end_report', `cash_tally_activity_report` = '$cash_tally_activity_report', `due_followup_customer_count_report` = '$due_followup_customer_count_report', `commitment_report` = '$commitment_report', `customer_status_report` = '$customer_status_report', `promotion_activity_report` = '$promotion_activity_report', `cleared_report` = '$cleared_report', `events_report` = '$events_report', `area_loan_count_report` = '$area_loan_count_report', `noc_handover_report` = '$noc_handover_report', `confirmation_count_report` = '$confirmation_count_report', `concern_report` = '$concern_report', `partners_report` = '$partners_report', `request_count_report` = '$request_count_report', `verification_count_report` = '$verification_count_report', `approval_count_report` = '$approval_count_report', `loan_issue_count_report` = '$loan_issue_count_report', `promotion_count_report` = '$promotion_count_report', `due_followup_count_report` = '$due_followup_count_report',  `back_office_count_report` = '$back_office_count_report', `branch_request_count_report` = '$branch_request_count_report', `location_track_report` = '$location_track_report',`outstanding_report` = '$outstanding_report', `reportmodule_intrest` = '$reportmodule_intrest', `intrest_ledger_report` = '$intrest_ledger_report', `intrest_loan_issue_report` = '$intrest_loan_issue_report', `intrest_collection_report` = '$intrest_collection_report', `intrest_balance_report` = '$intrest_balance_report', `intrest_closed_report` = '$intrest_closed_report', `search_module` = '$searchmodule', `search` = '$search_screen', `bulk_upload_module` = '$bulk_upload_module', `bulk_upload` = '$bulk_upload', `loan_track` = '$loan_track', `sms_module` = '$sms_module', `sms_generation` = '$sms_generation', `status` = 0, `update_login_id` = '$user_id', `updated_date` = current_timestamp() WHERE user_id = '$id' ";
	...
	public function addRequest($mysqli, $userid)
	{
		...
		try {
			...
			$select = $mysqli->query("SELECT autogen_cus_id FROM customer_register WHERE cus_id = '$cus_id' AND autogen_cus_id !='' AND autogen_cus_id IS NOT NULL");
			if ($select && $mysqli->affected_rows > 0) {
				$code = $select->fetch_assoc();
				$autogen_cus_id = $code['autogen_cus_id'];
			} else {
				// Find last auto ID
				$codeAvailable = $mysqli->query("
			...
			$mysqli->query("DELETE FROM enquiry WHERE cus_id = '$cus_id'") or die("Error " . $mysqli->error);
			$mysqli->query("DELETE FROM enquiry_promotion WHERE cus_id = '$cus_id'") or die("Error " . $mysqli->error);


			$insertQry = "INSERT INTO request_creation(`user_type`, `user_name`, `agent_id`, `responsible`, `remarks`, `declaration`, `req_code`, `dor`, `cus_id`, `cus_data`, `first_name`, `last_name`, `dob`, `age`, `gender`, `state`, `district`, `taluk`, `area`, `address`, `mobile1`, `mobile2`,`mobile_whatsapp`,`whatsapp_no`,`father_name`, `mother_name`, `marital`, `spouse_name`, `occupation_type`, `occupation`, `pic`, `loan_category`,  `tot_value`, `ad_amt`, `ad_perc`, `loan_amt`, `poss_type`, `due_amt`, `due_period`, `insert_login_id`,`created_date`) 
		VALUES('$user_type', '$user', '$agent', '$responsible', '$remarks',
		'$declaration', '$req_code', '$dor', '$cus_id', '$cus_data', '$first_name', '$last_name', '$dob', '$age', '$gender', '$state', '$district', '$taluk', '$area', '$address', '$mobile1', '$mobile2','$mobile_whatsapp','$whatsapp_no','$father_name', '$mother_name', '$marital', '$spouse_name', '$occupation_type','$occupation', '$pic', '$loan_category', '$tot_value', '$ad_amt', '$ad_perc','$loan_amt', '$poss_type', '$due_amt', '$due_period', '$userid', CURRENT_TIMESTAMP )";

			$mysqli->query($insertQry) or die("Error " . $mysqli->error);
			$req_ref_id = $mysqli->insert_id;

			if ($cus_data == 'New') {
				$CustomerInsert = "INSERT INTO customer_register (`autogen_cus_id`, `cus_id`,`req_ref_id`, `first_name`, `last_name`, `dob`, `age`, `gender`, `state`, `district`,`taluk`, `area`,  `address`, `mobile1`, `mobile2`,`mobile_whatsapp`,`whatsapp_no`, `father_name`, `mother_name`, `marital`, `spouse`, `occupation_type`, `occupation`,`pic`)
				VALUES( '$autogen_cus_id', '$cus_id','$req_ref_id','$first_name','$last_name','$dob', '$age', '$gender', '$state',
				'$district','$taluk','$area',  '$address', '$mobile1',
				'$mobile2','$mobile_whatsapp','$whatsapp_no','$father_name','$mother_name', '$marital', '$spouse_name',
				'$occupation_type','$occupation','$pic' )";
				$mysqli->query($CustomerInsert) or die("Error " . $mysqli->error); 
				
			} elseif ($cus_data == 'Existing') {
				$mysqli->query("UPDATE customer_register SET `autogen_cus_id` = '$autogen_cus_id', `cus_id` = '$cus_id',`req_ref_id` = '$req_ref_id', `first_name` = '$first_name', `last_name` = '$last_name', `dob` = '$dob', `age` = '$age', `gender` = '$gender', `state` = '$state', `district` = '$district', `taluk` = '$taluk', `area` = '$area',`address` = '$address', `mobile1` = '$mobile1', `mobile2` = '$mobile2', `mobile_whatsapp` = '$mobile_whatsapp', `whatsapp_no` = '$whatsapp_no', `father_name` = '$father_name', `mother_name` = '$mother_name', `marital` = '$marital', `spouse` = '$spouse_name', `occupation_type` = '$occupation_type', `occupation` = '$occupation', `pic` = '$pic', `cus_status` = 0 WHERE `cus_id`= '$cus_id' ");
			}

			for ($i = 0; $i < sizeof($category_info); $i++) {
				$insertQry = "INSERT INTO `request_category_info`(`req_ref_id`, `category_info`) VALUES ('$req_ref_id', '$category_info[$i]') ";
				$mysqli->query($insertQry) or die("Error " . $mysqli->error);
			}
	...
	public function updateRequest($mysqli, $id, $userid)
	{
		...
		$cus_data_upd = '';
		if (isset($_POST['cus_data_upd'])) {
			$cus_data_upd = $_POST['cus_data_upd'];
		}

		$cus_id_upd = '';
		if (isset($_POST['cus_id_upd'])) {
			$cus_id_upd = $_POST['cus_id_upd'];
		}
		...
		try {
			...
			//in request cus id can edit so there 4 scenerio inserted id -> edit id (New -> New => No issues, New -> Existing => Making duplicate, Existing -> New => Existing data will erase, Existing -> Existing => one cus data will erased and duplicate.) these are all while update by cus id without any condition.;
			if ($cus_id_upd != $cus_id) {
				$mysqli->query("SELECT * FROM customer_register WHERE cus_id = '$cus_id'");
				$cus_reg_cnt = $mysqli->affected_rows;
				...
				if ($cus_data_upd == "New") {
					...
					if ($cus_reg_cnt == 0) { //New
						...
						//...update by new cus id.
						$mysqli->query("UPDATE customer_register SET `autogen_cus_id` = '" . strip_tags($autogen_cus_id) . "', `cus_id` = '" . strip_tags($cus_id) . "', `req_ref_id` = '" . strip_tags($id) . "', `first_name` = '" . strip_tags($first_name) . "', `last_name` = '" . strip_tags($last_name) . "', `dob` = '" . strip_tags($dob) . "', `age` = '" . strip_tags($age) . "', `gender` = '" . strip_tags($gender) . "', `state` = '" . strip_tags($state) . "', `district` = '" . strip_tags($district) . "', `taluk` = '" . strip_tags($taluk) . "', `area` = '" . strip_tags($area) . "', `address` = '" . strip_tags($address) . "', `mobile1` = '" . strip_tags($mobile1) . "', `mobile2` = '" . strip_tags($mobile2) . "', `mobile_whatsapp` = '" . strip_tags($mobile_whatsapp) . "', `whatsapp_no` = '" . strip_tags($whatsapp_no) . "', `father_name` = '" . strip_tags($father_name) . "', `mother_name` = '" . strip_tags($mother_name) . "', `marital` = '" . strip_tags($marital) . "', `spouse` = '" . strip_tags($spouse_name) . "', `occupation_type` = '" . strip_tags($occupation_type) . "', `occupation` = '" . strip_tags($occupation) 	. "', `pic` = '" 	. 	strip_tags($pic) 	. "', `cus_status` = 0 WHERE cus_id= '$cus_id' ");
					}
	...
	public function addCustomerProfile($mysqli, $userid)
	{
		...
		$req_id_array = [];
		$req_result = $mysqli->query("SELECT req_id FROM request_creation WHERE cus_id = '$cus_id' AND cus_status BETWEEN 14 AND 17");
	...
	function addVerificationLoanCalculation($mysqli, $userid)
	{
		...
		if (isset($ad_amt) && isset($tot_value) && intVal($ad_amt) > 0 && intVal($tot_value) > 0) {
			$ad_per = number_format((intVal($ad_amt) / intVal($tot_value)) * 100, 1);
		} else {
			$ad_per = 0;
		}
	...
	public function addAcknowlegeDocumentation($mysqli, $userid)
	{
		...
		if (isset($_POST['doc_remark'])) {
			$doc_remark = $_POST['doc_remark'];
		}
		$doc_sts = 'NO';
		if (isset($_POST['doc_sts'])) {
			$doc_sts = $_POST['doc_sts'];
		}
		$replace_status = '1'; //0-YES, 1-NO, if block true only checkbox checked.
	...
	function getLoanList($mysqli, $id)
	{
		...
		// Getting Region Id, Branch ID, Branch Name
	...
	function addConcern($mysqli, $userid)
	{
		...
		try {
			...
			$mysqli->autocommit(FALSE);
			$myStr = "CC";
			$selectIC = $mysqli->query("SELECT com_code FROM concern_creation WHERE com_code != '' ORDER BY com_code DESC LIMIT 1 FOR UPDATE");
			if ($selectIC->num_rows > 0) {
				$row = $selectIC->fetch_assoc();
				$ac2 = $row["com_code"];
			...
			$insertQry = "INSERT INTO `concern_creation`( `raising_for`, `self_name`, `self_code`, `staff_name`, `staff_dept_name`, `staff_team_name`, `ag_name`, `ag_grp`, `cus_id`, `first_name`, `last_name`, `cus_area`, `cus_group`, `cus_line`, `com_date`, `com_code`, `to_dept_name`, `com_sub`, `com_remark`, `role_type`, `staff_assign_to`, `concern_creation_uploads`, `insert_user_id`) VALUES('$raising_for', '$self_name', '$self_code', '$staff_name', '$staff_dept_name', '$staff_team_name', '$ag_name', '$ag_grp', '$cus_id', '$first_name', '$last_name', '$cus_area', '$cus_group', '$cus_line', '$com_date', '$com_code', '$to_dept_name', '$com_sub', '$com_remark', '$role_type', '$staff_assign_to', '$concerncreation_upload', '$userid')";
	...
	function getCustomerRegister($mysqli, $cus_id)
	{
		...
		if ($mysqli->affected_rows > 0) {
			while ($row = $qry->fetch_assoc()) {
				...
				$detailrecords['dow'] = $row['dow'];
				$detailrecords['abt_occ'] = $row['abt_occ'];
				$detailrecords['reminder_call'] = $row['reminder_call'];
				// $detailrecords['cus_type'] = $row['cus_type'];
				// $detailrecords['cus_exist_type'] = $row['cus_exist_type'];
				$detailrecords['residential_type'] = $row['residential_type'];
				$detailrecords['residential_details'] = $row['residential_details'];
	...
	public function getDocumentDetails($mysqli, $cus_id)
	{
		...
		if ($mysqli->affected_rows > 0) {
			while ($row = $qry->fetch_assoc()) {
				...
				$detailrecords[$i]['doc_remark'] = $row['doc_remarks'];
				$detailrecords[$i]['doc_sts'] = $row['doc_sts'];
	...
	public function updateMortEndorse($mysqli, $userid, $type)
	{
		...
		$qry .= " `doc_sts`='" . strip_tags($doc_sts) . "',`doc_remarks`='" . strip_tags($doc_remark) . "' ";
	...
	//Bank Details Fetch
	public function getBankDetails($mysqli, $user_id)
	{
		$qry = "SELECT bc.* FROM bank_creation bc JOIN user u ON FIND_IN_SET(bc.id, u.bank_access) > 0 WHERE u.user_id =  $user_id; ";
		$res = $mysqli->query($qry) or die("Error in Get All Records" . $mysqli->error);
		$i = 0;
		$detailrecords = array();
		while ($row = $res->fetch_object()) {
	...
	public function updateCustomerProfile($mysqli, $userid)
	{
		...
		if (isset($_POST['reminder_call'])) {
			$reminder_call = $_POST['reminder_call'];
		}
		if (isset($_POST['cus_Tableid'])) {
			$cus_Tableid = $_POST['cus_Tableid'];
		}
		...
		$insertQry = "UPDATE in_verification SET `first_name` = '$first_name', `last_name` = '$last_name', `gender` = '$gender', `dob` = '$dob', `age` = '$age' 
		WHERE `cus_id` = '$cus_id' ";
		$insresult = $mysqli->query($insertQry) or die("Error " . $mysqli->error);
		...
		if (!empty($req_id_array)) {
			$cusUpd = "UPDATE `customer_profile` SET `cus_id` = '$cus_id', `first_name` = '$first_name', `last_name` = '$last_name', `gender` = '$gender', `dob` = '$dob', `age` = '$age', `whatsapp` = '$whatsapp_no', `cus_pic` = '$cus_pic', `residential_type` = '$cus_res_type', `residential_details` = '$cus_res_details', `residential_address` = '$cus_res_address', `residential_native_address` = '$cus_res_native', `occupation_type` = '$cus_occ_type', `occupation_details` = '$cus_occ_detail', `occupation_income` = '$cus_occ_income', `occupation_address` = '$cus_occ_address', `dow` = '$cus_occ_dow', `abt_occ` = '$cus_occ_abt', `area_confirm_type` = '$area_cnfrm', `area_confirm_state` = '$area_state', `area_confirm_district` = '$area_district', `area_confirm_taluk` = '$area_taluk', `area_confirm_area` = '$area_confirm', `latlong` = '$latlong', `area_group` = '$area_group', `area_line` = '$area_line', `update_login_id` = '$userid', `updated_date` = NOW() WHERE req_id IN (" . implode(",", $req_id_array) . " ) ";
			$updateCus = $mysqli->query($cusUpd) or die("Error " . $mysqli->error);


			$updateACkCus = "UPDATE `acknowlegement_customer_profile` SET `cus_id` = '$cus_id', `first_name` = '$first_name', `last_name` = '$last_name', `gender` = '$gender', `dob` = '$dob', `age` = '$age', `whatsapp` = '$whatsapp_no', `cus_pic` = '$cus_pic', `residential_type` = '$cus_res_type', `residential_details` = '$cus_res_details', `residential_address` = '$cus_res_address', `residential_native_address` = '$cus_res_native', `occupation_type` = '$cus_occ_type', `occupation_details` = '$cus_occ_detail', `occupation_income` = '$cus_occ_income', `occupation_address` = '$cus_occ_address', `dow` = '$cus_occ_dow', `abt_occ` = '$cus_occ_abt', `area_confirm_type` = '$area_cnfrm', `area_confirm_state` = '$area_state', `area_confirm_district` = '$area_district', `area_confirm_taluk` = '$area_taluk', `area_confirm_area` = '$area_confirm', `latlong` = '$latlong', `area_group` = '$area_group', `area_line` = '$area_line', `update_login_id` = '$userid', `updated_date` = NOW() WHERE req_id IN (" . implode(",", $req_id_array) . " ) ";
			$insresult = $mysqli->query($updateACkCus) or die("Error " . $mysqli->error);
		}
	}

	//Due followup reminder call. if customer said i will pay regularly on time so no need to call means mark as reminder call as "No", if not or customer not paid due properly then mark as yes. if no means customer not show in due followup list.
	public function updateReminderCall($mysqli){
		$cus_id =  preg_replace('/\s+/', '', $_POST['cus_id']) ?? '';
		$reminder_call = $_POST['reminder_call'] ?? '0';

		$updateCus = "UPDATE `customer_register` SET `reminder_call` = '$reminder_call' WHERE `cus_id` = '$cus_id' ";
		$mysqli->query($updateCus) or die("Error " . $mysqli->error);
	}
