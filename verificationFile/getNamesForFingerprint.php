    ...
    <tbody>
        <?php
            $qry = $connect->prepare("SELECT
                    ...
                    CONCAT(cr.first_name,' ', cr.last_name) AS name,
                ...
                WHERE cr.cus_id = ?
                GROUP BY cr.cus_id, CONCAT(cr.first_name,' ', cr.last_name)
                ...
                SELECT
                    ...
                    CONCAT(fi.first_name,' ', fi.last_name) AS name,
                    fi.relationship,
                    MAX(CASE WHEN f.hand = '1' THEN 'Added' END) AS left_hand,
                    MAX(CASE WHEN f.hand = '2' THEN 'Added' END) AS right_hand
                ...
                WHERE fi.cus_id = ?
                GROUP BY fi.relation_aadhar, CONCAT(fi.first_name,' ', fi.last_name), fi.relationship");
