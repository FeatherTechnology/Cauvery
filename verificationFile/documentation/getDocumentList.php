...
<table class="table custom-table" id='documentTable'>
    ...
    <tbody>
        ...
        $qry = $connect->query("SELECT ac.id,ac.doc_name,ac.doc_type,ac.doc_upload,ac.doc_holder,ac.holder_name,ac.`relation_name`,ac.`doc_info_upload_noc`,
        CONCAT(fam.first_name, ' ', fam.last_name) AS famname,fam.id from document_info ac Left JOIN verification_family_info fam ON ac.relation_name = fam.id where ac.req_id = $req_id and ac.doc_info_upload_used != '1' AND ac.doc_upload !=''");
