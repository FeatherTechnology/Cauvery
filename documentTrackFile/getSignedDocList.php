...
function getGuarentorName($connect, $req_id){
    $qry1=$connect->query("SELECT CONCAT(a.first_name, ' ', a.last_name) AS famname FROM `verification_family_info` a JOIN `acknowlegement_customer_profile` b on b.guarentor_name = a.id where b.req_id IN ($req_id)");
    $run=$qry1->fetch();
    return $run['famname'] ?? '';
}
?>
