...
<table class="table custom-table" id='documentTable'>
    ...
    <tbody>
        ...
        $k=1;
            ...
            $qry = $connect->query("SELECT ac.doc_name, ac.doc_type, ac.doc_upload, ac.doc_holder, ac.holder_name, ac.relation_name, CONCAT(fam.first_name, ' ', fam.last_name) AS famname FROM document_info ac LEFT JOIN verification_family_info fam ON ac.relation_name = fam.id WHERE ac.req_id IN ($req_id) ");
