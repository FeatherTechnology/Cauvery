...
$sql = $connect->query("SELECT req.req_id, req.cus_data, CASE 
        ...
        ELSE 0
    END AS has_closed_status, req.cus_id, cp.autogen_cus_id, CONCAT(cp.first_name,' ', cp.last_name) AS customer_name, al.area_name, bc.branch_name, agm.group_name, alm.line_name, cp.mobile1, req.cus_status AS consider_level, req.updated_date, np.status AS followup_sts, np.follow_date, np.followup_type, rcs.cus_status AS noc_cus_status $baseqry $limit");
