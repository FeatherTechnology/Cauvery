...
if($noc_member == 2){
    $qry = $connect->query("SELECT cp.guarentor_name,CONCAT(fam.first_name, ' ', fam.last_name) AS famname,fp.ansi_template , fp.hand FROM acknowlegement_customer_profile cp LEFT JOIN verification_family_info fam ON fam.id = cp.guarentor_name
