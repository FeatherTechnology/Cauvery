...
if ($type == 'today') {
    ...
    $qry = $connect->query("SELECT COALESCE(SUM(alc.doc_charge_cal), 0) AS doc_charge_cal, COALESCE(SUM(proc_fee_cal),0) AS proc_fee_cal FROM in_issue ii
                        ...
                        where DATE(ii.updated_date) = CURRENT_DATE AND  DATE(li.created_date) = CURRENT_DATE AND ii.cus_status > 13 $where ");
    $row = $qry->fetch();
    ...
    $qry = $connect->query("SELECT COALESCE(SUM(alc.doc_charge_cal), 0) AS doc_charge_cal, COALESCE(SUM(proc_fee_cal),0) AS proc_fee_cal FROM in_issue ii
                                JOIN acknowlegement_loan_calculation alc ON ii.req_id = alc.req_id   
                                JOIN loan_issue li ON li.req_id = ii.req_id   
                                where (DATE(ii.updated_date) >= DATE('$from_date') && DATE(ii.updated_date) <= DATE('$to_date')) AND (DATE(li.created_date) >= DATE('$from_date') && DATE(li.created_date) <= DATE('$to_date')) AND ii.cus_status > 13 $where ");
    $row = $qry->fetch();
    $response['doc_charge'] = $row['doc_charge_cal'];
    $response['proc_charge'] = $row['proc_fee_cal'];
} else if ($type == 'month') {
    ...
    $qry = $connect->query("SELECT COALESCE(SUM(alc.doc_charge_cal), 0) AS doc_charge_cal, COALESCE(SUM(proc_fee_cal),0) AS proc_fee_cal FROM in_issue ii
                                    JOIN acknowlegement_loan_calculation alc ON ii.req_id = alc.req_id  
                                    JOIN loan_issue li ON li.req_id = ii.req_id  
                                    where (MONTH(ii.updated_date) = '$month' && YEAR(ii.updated_date) = '$year') AND (MONTH(li.created_date) = '$month' && YEAR(li.created_date) = '$year') AND ii.cus_status > 13 $where ");
    $row = $qry->fetch();
    $response['doc_charge'] = $row['doc_charge_cal'];
    $response['proc_charge'] = $row['proc_fee_cal'];
}
